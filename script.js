...
var volSine = new Tone.Volume(-14);
var volPluck = new Tone.Volume(-15);
var volC = new Tone.Volume(-40);
...
}).chain(volC, pingPong, freeVerb, tremolo, channel, comp, Tone.Master);
...
var r = 0; var bash = 0; var cplusplus = 0; var csharp = 0; var nix = 0;
...
var canAddR = true; var canAddBash = true; var canAddCplusplus = true; var canAddCsharp = true; var canAddNix = true;
...
var onmessage = function (e) {
	...
	///////////////////////////////////////////////////
	if (canAddNix) {
		if (data.data.config.language === 'nix') {
			nix += 1;
		}
	}
	if (nix > 2) {
	...
	//////////////////////////////////////////
	if (canAddMinimal) {
		if (data.data.config.language === 'minimal') {
			minimal += 1;
		}
	}
	if (minimal > 2) {
		synthC.triggerAttackRelease('C2', "8n");
		minimal = 0;
		canAddMinimal = false;
		setTimeout(function() {
			canAddMinimal = true;
	...
	//////////////////////////////////////////
	if (canAddJulia) {
		if (data.data.config.language === 'julia') {
			julia += 1;
		}
	}
	if (julia > 2) {
		synthC.triggerAttackRelease('E2', "8n");
		julia = 0;
		canAddJulia = false;
		setTimeout(function() {
			canAddJulia = true;
	...
	//////////////////////////////////////////
	if (canAddGo) {
		if (data.data.config.language === 'go') {
			go += 1;
		}
	}
	if (go > 2) {
		synthC.triggerAttackRelease('G2', "8n");
		go = 0;
		canAddGo = false;
		setTimeout(function() {
			canAddGo = true;
	...
	//////////////////////////////////////////
	if (canAddCplusplus) {
		if (data.data.config.language === 'c++') {
			go += 1;
		}
	}
	if (go > 2) {
		synthC.triggerAttackRelease('B2', "8n");
		go = 0;
		canAddGo = false;
		setTimeout(function() {
			canAddGo = true;
